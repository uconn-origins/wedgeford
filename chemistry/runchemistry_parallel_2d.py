...
def main():
    ...
    #parser.add_option("-f", "--fraction", dest="fracrm", help="fraction of grain species to remove from final abundances", default = 0.0)
    ...
    #read in 2d inp flag from 0io file
    infile = open("5flags.inp" ,"r")
    lines = infile.readlines()
    infile.close()
    if lines[29].count("incl_2dabun     = 1") == 1:
    	rmoldinp = True
    	infile = open(environdir + options.iofil,"r")
    	lines = infile.readlines()
    	infile.close()
    	oldinp = lines[-1].split()
    	oldinp = oldinp[0]
    else:
    	rmoldinp = False

    print( "remove old inp? ", rmoldinp)
    ...
    rundc.write("./disk_chemistry " + environdir + options.iofil + " " + environdir + "1environ.inp."+options.dust+".$1 > code.out.r$1.out\n")
    rundc.write("mv " + basedir + '/r$1_' + options.dust + '_*.out ' + outdir + options.dust + '/\n')
    rundc.write("mv " + basedir + '/code.out.r$1.out ' + outdir + options.dust + '/\n')
    rundc.write("mv " + basedir + '/*_$1*.rout ' + outdir + options.dust + '/\n')
    rundc.write("cp " + basedir + '/r$1_' + options.dust + '_*.inp ' + outdir + '/\n')
    if rmoldinp == True:
    	rundc.write("rm " + basedir + '/r$1_' + options.dust + '_*_'+ oldinp+'.inp\n')
    	print("removed old inp")
    rundc.write("./streetsweeper.py -o " + outdir + options.dust + '/' + ' -e ' + environdir + "1environ.inp."+options.dust+".$1\n")
    rundc.write("echo R=$1 completed.")
    rundc.close()
