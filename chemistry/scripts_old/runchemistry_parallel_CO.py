...
def main():
    ...
    parser.add_option("-c", type=float, dest="xco", help="CO initial abundance", default=1e-4)
    parser.add_option("--nit", type=str, dest="nit", help="Nitrogen Variant Init Cond, '.kf', '.kw', or none ('').", default='')
    ...
    # Now change the 3abunds.inp file
    xco = options.xco
    xh2o = 2.500e-04+1e-4-xco
    print "cp -f 3abunds.inp.tw"+nit+" 3abunds.inp.tmp; rm -f 3abunds.inp 3abunds.inp.tmp2"
    #return
    os.system("cp -f 3abunds.inp.tw"+nit+" 3abunds.inp.tmp; rm 3abunds.inp")
    os.system('cat 3abunds.inp.tmp | sed "s/INPVAL/'+"%7.3e"%(xco)+'/" > 3abunds.inp.tmp2')
    os.system('cat 3abunds.inp.tmp2 | sed "s/INPWAT/'+"%7.3e"%(xh2o)+'/" > 3abunds.inp')
