...
C If using 2D initial abundances read them in now        	
        	if (incl_2dabun .eq. .TRUE.) then
      			open (unit=01,file=abun2dfile)
      			fend = 0
				nfrc = 0
				do while (fend .EQ. 0)
					read(01,'(A160)',iostat=fend) line
...
c$$$                lastyr = tlast/year
c$$$                WRITE (lts,'(1PE10.1)') lastyr
c$$$                write(*,*) lts
c$$$                OPEN (unit=21,file=trim(out)//'_'//ait(1:nait)//'_'
c$$$     &               //trim(adjustl(lts))//'.inp',status='unknown',
c$$$     &               access='append')
c$$$                REWIND 21
c$$$                do j = 1, nre     
c$$$                   write(21,66) s(j),abundances(j,l)/gdens
c$$$                enddo
c$$$ 66             format(1a13,1pe11.3)
c$$$                CLOSE(21)
