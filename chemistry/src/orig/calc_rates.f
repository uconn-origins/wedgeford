...
C External Functions
	DOUBLE PRECISION self_shield_CO, self_shield_H2, grain_abun_check
	DOUBLE PRECISION photorate, photodesorp_rate, photorate_nocalc

C   for type 41
	REAL Nsites, nsfsites, a_spacing, v_zeri, v_zerj, t_hop_i
	DOUBLE PRECISION t_hop_j, R_diff_i, R_diff_j, R_diff_i_q, ndustdens,t_quant
...
C     omega = grain albedo
	ELSE IF (rtype .EQ. 15) THEN
		...
		xh2 = 0.5
		omega = 0.5
		calcrate = 2.6 * psi * alpha * xh2 * xrayrate * 1.0/(1-omega)
...
C	  gamma = E_act
 	...
 	ELSE IF (rtype.EQ.41) THEN
	    ...
	    kappa_ij = exp(-2*(a_spacing/(hp/(2*pi)))*(2*mu_i_j*kbol*gamma)**0.5)  ! 
...
C	  gamma = E_act
 	...
 	ELSE IF (rtype.EQ.42) THEN
		...
		mu_i_j = mass_i*mass_j/(mass_i+mass_j)*mH
	    kappa_ij = exp(-2*(a_spacing/(hp/(2*pi)))*(2*mu_i_j*gamma)**0.5)  ! 
		...
		select case(strg1)
         	case('H(gr)')
			t_quant = v_zeri*exp((2*a_spacing/(hp/(2*pi)))*(2*mass_i*alpha*0.3)**0.5)
			R_diff_i_q = 1.0/(Nsites*t_quant)
...
C..............................................................................
	...
	PARAMETER (kTmin = 1.0, kTmax = 10.0)
...
C Common Blocks
	...
	DOUBLE PRECISION KTXR, E1, E2, Nsec
	PARAMETER (KTXR = 2, E1 = 0.8, E2 = 10.0, Nsec = 30)

C External Functions
	DOUBLE PRECISION sigmaE, Habssigma
	
c bug with integral
	Lv0 = Lxray/KTXR / (dexp(-E1/KTXR) - dexp(-E2/KTXR))  
c Jeffrey: 	Lv0 = Lxray/KTXR * (dexp(-E1/KTXR) - dexp(-E2/KTXR))
	Lv = Lv0 * dexp(-E/KTXR)
c Jeffrey: 	F0 = Lv/(8*pi*Rs(zone)**2*AU**2 * E * keV)
	F0 = Lv/(8*pi*Rs(zone)**2*AU**2 * E * keV)
	
	xrayinteg = Nsec * Habssigma(E) * F0 * dexp(-sigmaE(E)*Nrz(zone))
...
C..............................................................................
	...
	cgas = reshape ( (/ 17.9, 37.0, 49.3, 58.4, 49.2, 78.0, 79.8, 117.0, 
     &						109.0, 108.0, 138.0, 142.0, 139.0, 95.9,
	...
	sigmagas = (cgas(j,1) + cgas(j,2) * Einp + cgas(j,3) * Einp**2)
	sigmadust = (cdust(j,1) + cdust(j,2) * Einp + cdust(j,3) * Einp**2)
...
C http://physics.nist.gov/cgi-bin/Xcom/xcom2
C for photoelectric absorption of H
C
C..............................................................................
	DOUBLE PRECISION FUNCTION Habssigma(Einp)
	
	DOUBLE PRECISION sigmavals(9), Evals(9), Einp
	INTEGER i, j
	...
	do while ((d .ne. '').and.(d .ne. '('))
         ...
         select case(d)
			...
			if (wasdigit) then
			    ...
			    atomarr(oldatomindex) = atomarr(oldatomindex)-xold+numberofatoms
		        oldnumberofatoms = numberofatoms
				wasdigit = .true.			    
			else
			    read( d, '(i10)' )  x
			    numberofatoms = x
			    atomarr(oldatomindex) = atomarr(oldatomindex) + numberofatoms-1
		        oldnumberofatoms = numberofatoms
				wasdigit = .true.
			endif
