...
C copyright (c) 2008-10, Jeffrey Fogel, University of Michigan
C
C..............................................................................
...
C		readr, reads, readuv, run_chemistry
C
C..............................................................................
...
C Global variable(s):
     ...
     1  yy, gdens
      ...
      CHARACTER*80 specs, rates, out, ver, cmgtm, uvfile
      DIMENSION frc(nspec2), spec0(nspec2), yy(nspec)

C External Functions
	  DOUBLE PRECISION xrayion_rate, calcrate
...
C Current version of the code:
      PARAMETER (ver = ' version 2.7 (1/10/2011)')
...
C Parameters
			if (trim(flagname).EQ.'g0_100') then
				read(flagval, '(E8.2)') g0_100
			else if (trim(flagname).EQ.'g0_CO') then
				read(flagval, '(E8.2)') g0_CO
			else if (trim(flagname).EQ.'Lxray') then
				read(flagval, '(E8.2)') Lxray
			else if (trim(flagname).EQ.'fg') then
...
c Determine a numerical dust value
	  tmpdust = dust
	  DO i=1,len_trim(dust)
		IF (dust(i:i) .EQ. 'e') THEN
			tmpdust(i:i) = ' '
		ELSE IF (dust(i:i) .EQ. 'p') THEN
			tmpdust(i:i) = '.'
		END IF
	  ENDDO
	  read(tmpdust, *) ndust
...
C Calculate the x-ray ionization rate for this zone
		xrayrate = xrayion_rate()
...
C ODE solver there
		IF (zAU(zone) .GT. ZMAX .OR. Rs(zone) .GT. RMAX) THEN
			DO j=1,nspec
				yy(j) = 0.0
			ENDDO
C Ignore cells with Temperature = 0 or Density = 0
        ELSE IF (Tg(zone).EQ.0 .OR. Td(zone).EQ.0 .OR. rho(zone).EQ.0) THEN
			DO j=1,nspec
				yy(j) = 0.0
			ENDDO
C Run zone
