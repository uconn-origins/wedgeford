...
#zeta_powerlaw alpha zeta_exponential Sigma
    ...
    mumult = 2.36
    piv = 1e20
    F = zp*ze*mumult/(ze*mumult*(fineN/piv)**al+zp*(exp(fineN*2.0*1.6733e-24/co)-1)) #C13 eq5
    return F
...
def mainTG(baseenvdir,destenvdir,Tgasfile):
     ...
     for e in range(1):
          ...
          print("Tgasmod",baseenvdir+Tgasfile)
          ...
          for i in range(nR):
               ...
               TgasT =TgasT[0]
               print("R",radstr[i],baseenvdir+'1environ.inp.e1.'+radstr[i])
               print("0TgasZ",TgasZ)
               TgasZ = TgasZ[0]
...
diskchem_dir = '/home/kschwarz/MasterChemistry'
...
drift = False
srt_time_vals = ['None']#,'None','None','None','None','None','None','None','None']
...
xr_vals = ['30.0']
isrf_vals = ['1']#,'10000.0']#,'300.0','3000.0']
...
for m in range(len(mass)):
    for l in range(len(lg)):
        ...
        env_orig.append(extraflag[m]+mass[m]+lg[l])
        short_env_orig.append(mass[m]+lg[l])
        #short_env_orig.append(mass[m])
        uv_model.append('uv_photons_'+extraflag[m]+mass[m]+lg[l]+'_uv_e1.dat')
        #uv_model.append('uv_photons_'+extraflag[m]+mass[m]+'_uv_e1.dat')
        slr_model.append('None')
        srt_time.append('None')
print(env_orig)
print(short_env_orig)
print(uv_model)
print(slr_model)
print(srt_time)
#sys.exit()
...
for x in range(len(xr_vals)):
	for e in range(len(env_orig)):
		xr_model = 'xray_phot_'+short_env_orig[e]+'_xr_Lx'+xr_vals[x]+'.dat'
